...
gtk_plot_canvas_motion (GtkWidget *widget, GdkEventMotion *event)
{
  ...
  switch(canvas->action){
    ...
    case GTK_PLOT_CANVAS_MOVE_TITLE:
         ...
         new_x = text_x + (x - canvas->drag_x); 
         new_y = text_y + (y - canvas->drag_y); 
         gdk_draw_rectangle (GTK_LAYOUT(canvas)->bin_window,
                             xor_gc,
                             FALSE,
                             new_x, new_y,
    ...
    case GTK_PLOT_CANVAS_DND_POINT:
         if(active_dataset == NULL) return TRUE; 
         if(active_point == NULL) return TRUE; 
	 ...
	 gtk_plot_pixel_get_point(active_plot, x, y, &point);

	 gtk_plot_draw_dataset(active_plot, xor_gc, active_dataset);

	 active_point->x = point.x;
         active_point->y = point.y;
...
gtk_plot_canvas_button_press(GtkWidget *widget, GdkEventButton *event)
{
  ...
  if(GTK_PLOT_CANVAS_CAN_MOVE_TEXT(canvas)){ 
    while(text)
     {
  ...
  while(plots)
     {
       ...
       if(GTK_PLOT_CANVAS_CAN_MOVE_TITLES(canvas)){ 
         for(i = 0; i <= 3; i++){
  ...
  while(plots)
     {
       ...
       if(GTK_PLOT_CANVAS_CAN_MOVE_PLOT(canvas)){ 
       ...
       if(GTK_PLOT_CANVAS_CAN_RESIZE_PLOT(canvas)){ 
         if((y >= py+pheight-5 && y <= py+pheight+5) && 
    ...
    while(dataset)
     {
       ...
       for(i=0; i<data->num_points; i++)
        {
         point = &data->points[i];
         gtk_plot_point_get_pixel(active_plot, *point, &xi, &yi);
         if(abs(xi-x) <= 5 && abs(yi-y) <= 5)
          {
              canvas->active_data = data;
              canvas->active_point = point;
  ...
  if(!veto) canvas->action = GTK_PLOT_CANVAS_INACTIVE;
...
GtkPlotPoint *
gtk_plot_canvas_get_active_point(GtkPlotCanvas *canvas)
{
