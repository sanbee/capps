...
//---------------------------------------------------------------------
      else if (!strcmp(KeyWord,              "xscale"))
	{
	  ...
	  if ((v=strtok(NULL,","))) sscanf(v,"%f",&base);
	  ...
	  if (Val)
	    {
	      ...
	      try {Doer->SetAttribute(XYPanel::TITLE,l,NULL,n);}
	      catch (ErrorObj x) {x<<x.what()<<endl;}
	    }
	}
//---------------------------------------------------------------------
...
int Hub::CtrlProcessor(int fd, void *data)
{
  ...
  //
  sprintf(Msg,"%d%c",Com1.port(),NULL);
  ...
  // after sending the ACK.
  //
  sprintf(Msg,"%d%c",Com2.port(),NULL);
  //  cerr << "Sending..." << Msg << endl;
...
int Hub::GulpASCIIData()
{
  ...
  for (int i=0;i<NoOfCols;i++)
    {
      if (i == 0) sscanf(strtok(Msg," "),"%f",&D);
      else        sscanf(strtok(NULL," "),"%f",&D);
