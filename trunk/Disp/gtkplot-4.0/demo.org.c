...
gint key_press_handler(GtkWidget* widget, GdkEventKey *event, 
			 gpointer func_data)
{
  gint x,y;
  gtk_widget_get_pointer(widget, &x, &y);

  fprintf(stderr,"Key pressed. String=\"%s\", Val=%d At=%d %d\n",
	  event->string,event->keyval,x,y);

  if (event->string[0] == 'p')
    {
      fprintf(stderr,"Writing the current display to plotdemo.ps\n");
...
build_example1(GtkWidget *active_plot)
{
 ...
 gtk_plot_dataset_set_points(dataset[0], px1, py1, dx1, dy1, 6);
 ...
 gtk_plot_dataset_set_points(dataset[3], px2, py2, dx2, dy2, 6);
...
*/
 ...
 gtk_plot_dataset_set_points(dataset[2], px2, py2, dx2, NULL, 8);
...
int main(int argc, char *argv[]){
 ...
 gtk_rc_add_default_file("demo.rc");
 gtk_init(&argc,&argv);


 window1=gtk_window_new(GTK_WINDOW_TOPLEVEL);
 gtk_widget_set_name (window1, "main window");
 ...
 gtk_signal_connect (GTK_OBJECT (window1), "key_press_event",
		     GTK_SIGNAL_FUNC (key_press_handler), NULL);
 ...
 gtk_widget_set_name (scrollw1, "canvas");
...
*/
 ...
 gtk_widget_set_name (layout, "plot1");
 ...
 gtk_plot_layout_put_text(GTK_PLOT_LAYOUT(canvas), .40, .005, 0, 
