...
void MultiPanel::Redraw(int AutoScale, int WhichPanel,int WhichOverlay)
{
  float Range[2];
  int Width, Height;
  gtk_widget_get_size(GTK_WIDGET(TopLevelWin),&Width, &Height);
  ...
  //  gtk_plot_paint(GTK_WIDGET(Layout),NULL);
				  ...
				  int Disp, char *Title)
{
  ...
  gtk_box_pack_start (GTK_BOX (TopLevelVBox), menubar, FALSE, TRUE, 0);
