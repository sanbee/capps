				     ...
				     IMAGETYPE<T> &ModelVis)
{
  ...
  //
  // Make model visilibites (FT(MComp))
  //

  Convo.toFTDomain(ResImg,ModelVis);
  ModelVis.size(Nx,Ny);

  //  sendImage(*Global::ClientPtr,Global::EventPtr,Vis,"vis");
  //  sendImage(*Global::ClientPtr,Global::EventPtr,ModelVis,"mvis");
  
  //
  // Replace the ModelVis by the residual visibilites (Vis-ModelVis).
  //
  for (int i=0;i<Nx;i++)
    for (int j=0;j<Ny;j++)
      ModelVis.setVal(i,j,Vis(i,j)-ModelVis(i,j));

  //  sendImage(*Global::ClientPtr,Global::EventPtr,ModelVis,"rvis");

  //
  // Make the Residual image (iFT(ModelVis)
  //
...
template <class T> void ToComplex(IMAGETYPE<T> &inImg, IMAGETYPE<T> &outImg)
{
  int Nx,Ny;
  inImg.size(Nx,Ny);
  outImg.resize(Nx/2,Ny);

  for (int i=0;i<Nx/2;i++)
    for (int j=0;j<Ny;j++)
      outImg.setVal(i,j,inImg(2*i,j));
}
					 ...
					 IMAGETYPE<T>& RestoredImg)
{
  ...
  Convolver<T> lConvo(CleanPSF);
  //  ResComps = CCList.convolve(CleanBeam);
  //  RestoredImg += ResComps;
  RestoredImg += CCList;
  lConvo.convolve(RestoredImg);
...
//---------------------------------------------------------------------------------------------
//
template <class T> void TellTheWorld(IMAGETYPE<T> &CCImg,
				     IMAGETYPE<T> &ResImg, 
				     FluxonCompList<T> &CC,
...
template <class T> static T my_f (const gsl_vector *v, void *params)
{
  ...
  //
  // Make ResImg vis FT(ResVis) (ResVis here is in ModelVis)
  //
  (*MImg).size(NX,NY);
  (*MImg).assign(0);
  ...
  // 
  // Make model Dirty Image via approximate analytical convolution
  //
  //  (*ModelDI) = (*MComp).convolve((*PSF));
  //  (*MImg) += (*ModelDI);

  //
  // Make ResImg via FFT convolution
  //
  (*MImg).size(NX,NY);
  (*MImg).assign(0);
  (*MImg) += (*MComp);
  ...
  sendImage(*Global::ClientPtr,Global::EventPtr,(*ResImg),"restoredimg");
  //
  // Compute the ChiSq
  //
  ...
  {
    FTYPE tChisq=0;    int Nx,Ny;
    Vis.size(Nx,Ny);
    for (int i=0;i<Nx;i++) 
      for (int j=0;j<Ny;j++) 
	tChisq += ModelVis(i,j)*ModelVis(i,j);
	  ...
	  for (int i=0;i<Pi;i++) 
	    {
	      ...
	      cerr << "dAsp: " << i << " " << d << " " << PIndex[i] << " " << MI[i] << " " << MI[i].narea() << endl;
	    }
	  cerr << "done!";
...
//	gsl_vector_set(x, m++, gsl_vector_get(x0,2));
	...
	*/
	...
	for (int j=0;j<(NY);j++) 
      ...
      //      TellTheWorld(MImg,ResImg,MI,PSF);
...
template<class T> int sendImage(Client &c, GlishEvent *e,
				 IMAGETYPE<complex<T> >& Img, char *EventName=NULL)
{
  T *ptr;
  ptr = (T *)Img.getStorage();
  int Nx, Ny;
  Img.size(Nx,Ny);

  cerr << "Complex image of size " << Nx << " " << Ny << endl;

  Value *Reply = new Value(ptr, 2*Nx*Ny, COPY_ARRAY);
