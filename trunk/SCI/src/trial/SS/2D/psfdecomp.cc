				   ...
				   )
{
  ...
  *Solver = gsl_multimin_fdfminimizer_alloc(MinimizerType, NParams);
  //  cerr << "Using " << gsl_multimin_fdfminimizer_name(*Solver) << " minimization" << " for " << NParams << " params" << endl;
  ...
  for(int i=0;i<Pi;i++)
	...
	*/
      ...
      if ((fabs(OneComp[0].getAmp()) < Threshold))
	{
	  ...
	  if (f0.getScale() > 0.05) f0.setScale(f0.getScale()/1.4);
	  tmp += f0;
	  Convo.makeXFR(tmp);
	  SmoothPSF = PSFImage;
