				     ...
				     int N0, int N1,int PSFSize)
{
  ...
  for (int Pi=N0; Pi<N1; Pi++)
	...
	*/
	if (tx) gsl_vector_free(tx);
      ...
      MImg   += ModelDI;
      */
      cerr << "###Info: Doing major cycle..." << endl;
      MImg.assign(0.0);
      MImg += MI;
      Convo.convolve(MImg);
      /*
      {
	ofstream tst("junk.img");
	tst << MImg;
	exit(0);
      }
      */
  ...
  if (N0==N1)
	...
	if (MI.size() > i)
      ...
      MImg   += ModelDI;
      for (int i=EDGE;i<(NX-EDGE);i++) 
	for (int j=EDGE;j<(NX-EDGE);j++) 
	  ResImg.setVal(i,j,(DImg[i] - MImg[i]));
...
int main(int argc, char **argv)
{
  ...
  while (e=c.NextEvent())
	...
	{
	  cerr << "###No. of PSF Comps written " << writeComps(c, e, CCList) << endl;
	}
