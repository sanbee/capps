...
public:
  ...
  enum Derivatives {DA=0, DS=1, DXP=2, DYP=3};
  Fluxon2D<T>(Exp<T>& ExpTable=GlobalParams::TableOfExponential) 
  {
    itsA=1.0; itsXPos=itsYPos=0; itsScale=1.0;
  ...
  {
    itsA=A; itsXPos=XPos; itsYPos=YPos; itsScale=Scale;
  ...
  inline void setScale(T Scale)            {itsScale=(itsScale=abs(Scale))>ZERO_SCALE?ZERO_SCALE:itsScale;}
  ...
  inline int inRange(T x, T y, int nx0, int nx1, int ny0, int ny1) 
  {return ((x >= nx0) && (x <nx1) && (y >= ny0) && (y < ny1));}
  //
  // Assignment operator
  //
  /*
...
#define ATAN_MAX 1.5707
#define ATAN_MAX2 3.1415
  inline T scale (T s,T maxS=MAX_SCALE) {return (maxS/2)*(1+atan(s)/ATAN_MAX);}
  inline T dScale(T s,T maxS=MAX_SCALE) {return (maxS/ATAN_MAX2)*(1.0/(1+s*s));}
  ...
  inline T operator()(T x,T y) 
  {
    ...
    arg = ((x-getXPos())*(x-getXPos())+(y-getYPos())*(y-getYPos()))*getScale()*getScale();
  ...
  inline void evalDerivatives(T x, T y) 
  {
    ...
    itsDerivatives[DA]  =  -tmp/(itsA);                     // Sum[-2*Res * P_k/A_k]
    itsDerivatives[DS]  =   (x2*x2 + y2*y2)*itsScale*tmp;   // Sum[ 2*Res * ([x-x_k]^2 + [y-y_k]^2) * Scale * P_k]
#if FITPOS
    itsDerivatives[DXP] =  -x2*itsScale*itsScale*tmp;       // Sum[-2*Res * [x-x_k] * Scale^2 * P_k]
    itsDerivatives[DYP] =  -y2*itsScale*itsScale*tmp;       // Sum[-2*Res * [y-y_k] * Scale^2 * P_k]
#endif
  }
  inline T dF(Derivatives D) {return itsDerivatives[D];}
  
private:
  T itsA, itsXPos, itsYPos, itsScale, itsXShift,itsYShift;;
