...
void cacheATerm(cuComplex* Ad_buf_p, const Float Freq, const Float pa, 
		const Int bandID, AntennaATerm& aTerm)
{
  CoordinateSystem skyCoords(aTerm.skyCoords());
  IPosition skyShape(aTerm.skyShape());
  CoordinateSystem uvCoords = SynthesisUtils::makeUVCoords(skyCoords,skyShape);

  Int index = uvCoords.findCoordinate(Coordinate::LINEAR);
  LinearCoordinate lc=uvCoords.linearCoordinate(index);
  Vector<Double> uvIncr = fabs(lc.increment());

  TempImage<Complex> thePB(skyShape,skyCoords);
...
int main(int argc, char *argv[])
{
  ...
  aTerm.setSkyCoords(thisAVGPB.coordinates(), skyShape);
  ...
  atimer.mark();
  //  cudaProfilerStart();
  cacheATerm(Ad_buf_p, Freq, pa, bandID,  aTerm);
  //  cudaProfilerStop();
