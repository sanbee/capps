...
namespace casa{
  ...
  void BeamCalc::Efield(const calcAntenna *a, const Complex *pol, Complex *E)
  {
    ...
    for(Int i = 0; i < 3; i++)
      E[i] = make_cuFloatComplex(hhat[i],0)* pol[0] + make_cuFloatComplex(vhat[i],0)*pol[1];
  }
  ...
  void BeamCalc::tracepol(Complex *E0, const Ray *ray, Complex *E1)
  {
    ...
    norm3(r); 
    fac = make_cuFloatComplex(r[0],0)*E1[0] + make_cuFloatComplex(r[1],0)*E1[1] + make_cuFloatComplex(r[2],0)*E1[2];
    for(i = 0; i < 3; i++) E1[i] = make_cuFloatComplex(r[i],0)*fac*2.0 - E1[i];
  ...
  Int BeamCalc::calculateAperture(ApertureCalcParams *ap)
  {
    ...
    Complex Iota; Iota=make_cuFloatComplex(0,1);
    ...
    if(obsName_p=="EVLA" || obsName_p=="VLA"){
      ...
      //      Pr[0] = make_cuFloatComplex(1.0/M_SQRT2,0.0); Pr[1] =  make_cuFloatComplex(Iota/M_SQRT2,0.0);
      Pr[0] = make_cuFloatComplex(1.0/M_SQRT2,0.0); 
      cuComplex tmp=make_cuFloatComplex(M_SQRT2,0.0);
      tmp = Iota/tmp;
      Pr[1] = negate(tmp);
      //      Pr[1] =  negate(make_cuFloatComplex(Iota/(M_SQRT2),0.0));
      ...
      q[0] = Pr[0];
      q[1] = Pr[1];
      Pr[0] =  make_cuFloatComplex(cp,0)*q[0] + make_cuFloatComplex(sp,0)*q[1];
      tmp=make_cuFloatComplex(sp,0);
      Pr[1] = negate(tmp)*q[0] + make_cuFloatComplex(cp,0)*q[1];
      q[0] = Pl[0];
      q[1] = Pl[1];
      Pl[0] =  make_cuFloatComplex(cp,0)*q[0] + make_cuFloatComplex(sp,0)*q[1];
      tmp=make_cuFloatComplex(sp,0);
      Pl[1] = negate(tmp)*q[0] + make_cuFloatComplex(cp,0)*q[1];
    }
    else{
      /* in linear basis */
      Pr[0] = make_cuFloatComplex(1.0,0.0); Pr[1] = make_cuFloatComplex(0.0,0.0);
      Pl[0] = make_cuFloatComplex(0.0,0.0); Pl[1] = make_cuFloatComplex(1.0,0.0);
    }
    ...
    Efield(a, Pr, Er); 
    Efield(a, Pl, El); 
    ap->aperture->set(make_cuFloatComplex(0.0,0.0));
...
#pragma omp for
  ...
  Int BeamCalc::calculateAperture(ApertureCalcParams *ap, const Int& whichPoln)
  {
    ...
    engine_computeBeamPixelValues(ap, a, p, L0, Er, El, i,j,nx,ny,whichPoln);
...
#ifdef cuda_kernel
  void BeamCalc::engine_computeBeamPixelValues(const ApertureCalcParams *ap, 
					       const calcAntenna *a, const Pathology *p,
					       const Double L0,
					       Complex *Er, Complex *El,
					       const Int dummyi, const Int dummyj,
...
#define TILE_WIDTH 32
	...
	//  	for(Int i = 0; i < nx; i++)
	    ...
	    Int i,j;
	    BeamCalc *myBC=BeamCalc::Instance();
	    kernel_computePixelValues<<<dimGrid, dimBlock>>>(myBC, ap, a, p, L0, Er, El, i,j,whichPoln,TILE_WIDTH);
  	  }
      }
  }
#else
...
#pragma omp for
  ...
  __global__ void kernel_computePixelValues(BeamCalc* beamCalc, const ApertureCalcParams *ap, 
					    ...
					    const int tileWidth)
  {
    Int ii = tileWidth*blockIdx.x + threadIdx.x ;
    Int jj = tileWidth*blockIdx.y + threadIdx.y ;
    ...
    //   for(Int i=0; i < nx; ++i)
    {
      ...
      for(Int iter = 0; iter < niter; iter++)
	{
	  ray = beamCalc->trace(a, x1, y1, p);
	  if(!ray) goto nextpoint;
	  x1 += (x - ray->aper[0]);
	  y1 += (y - ray->aper[1]);
	  beamCalc->deleteRay(ray);
	  ray = 0;
	}
      ...
      /* check for leg blockage */
      if(beamCalc->legplanewaveblock2(a, ray))
	goto nextpoint;
      if(beamCalc->legsphericalwaveblock(a, ray))
	goto nextpoint;
      
      if(y < 0) rayy = beamCalc->trace(a, x1, y1+eps, p);
      else rayy = beamCalc->trace(a, x1, y1-eps, p);
      
      if(x < 0) rayx = beamCalc->trace(a, x1+eps, y1, p);
      else rayx = beamCalc->trace(a, x1-eps, y1, p);
	...
	goto nextpoint;
      ...
      dA = 0.5*fabs(dx1*dy2 - dx2*dy1);
      dO = (beamCalc->dOmega(a, rayx, rayy, ray, p)/dA)*dx*dx;
      dP = dO*beamCalc->feedgain(a, ray, p);
      amp = sqrt(dP);
      
      L = beamCalc->Raylen(ray);
	...
	{tmp=ap->aperture->getAt(pos);ap->aperture->putAt(tmp+ll,pos);}
    nextpoint:
      if(ray)  beamCalc->deleteRay(ray);
      if(rayx) beamCalc->deleteRay(rayx);
      if(rayy) beamCalc->deleteRay(rayy);
    }
  };
#endif
