			 ...
			 gint makeYScrollBars)
{
 ...
 gtk_widget_set_name (CHART, name);
  ...
  //  gtk_plot_axis_set_visible(GTK_PLOT(CHART), GTK_PLOT_AXIS_BOTTOM, TRUE);
  //
  // Hide ticks and labeling of the top and right axis
  //
  gtk_plot_axis_show_labels (GTK_PLOT(CHART), GTK_PLOT_AXIS_TOP,    0);
  gtk_plot_axis_show_labels (GTK_PLOT(CHART), GTK_PLOT_AXIS_RIGHT,  0);
  //
  // Draw the x=0 y=0 lines
  //  
  gtk_plot_set_x0_visible(GTK_PLOT(CHART), TRUE);
  gtk_plot_set_y0_visible(GTK_PLOT(CHART), TRUE);
  //
  ...
  if (Yi[WhichOverlay] == NULL)
    {
      ...
      Yi[WhichOverlay] = gtk_plot_dataset_new();
...
// gtkplot.c) has been removed since that causes un-necessary calls to
...
void XYPanel::Plot(int WhichOverlay)
{
  ...
  if (!(GTK_PLOT_FLAGS(CHART)))
	...
	throw(ErrorObj("Requested overlay for plot does not exist",
      ...
      //
      // The following re-draws the given CHART, but blanks out the rest of the canvas.
      // In gtk_plot_layout_refresh_chart, the GdkPixmap must be first copied.
      //
      //      gtk_plot_layout_refresh_chart(GTK_PLOT_LAYOUT(Canvas),GTK_PLOT(CHART));
    }
}
//
//-------------------------------------------------------------------
		      ...
		      int Angle)
{
  ...
  gtk_plot_put_text(GTK_PLOT(CHART),
		    ...
		    //		    0,//Justification
		    GTK_JUSTIFY_CENTER,//Justification
		    Text); 
}
//
//-------------------------------------------------------------------
...
{
  //  cerr << "DeleteText: Functionality commented out due to move to GTKPlot 5.0" << endl;
  ...
  if (delta)
    {
      ...
      if ((P > 3) || (P <= -3)) {Precision=(int)fabs(fabs(P)-2); Style = GTK_PLOT_LABEL_EXP;}
  ...
  if (YMax_Adj->value > YMin_Adj->value)
    {
      ...
      Plot();
      // GtkPlotLayout *l=GTK_PLOT_LAYOUT(Canvas);
      // gtk_plot_layout_refresh_chart(l,GTK_PLOT(CHART));
    }
  else
