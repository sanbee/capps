...
void
build_example1(GtkWidget *active_plot,gfloat Scale)
{
  GdkColor color;
  gint i;
  static gint d=0;
  ...
  /*
  for (i=0;i<6;i++) points[i].y *= Scale;
  for (i=0;i<6;i++) points3[i].y /= Scale;
  */
  gtk_plot_set_range (GTK_PLOT(active_plot),
  ...
  if (!d)
    {
      ...
      gtk_plot_dataset_set_name(dataset[3],NULL);
    ...
    gtk_widget_queue_draw(GTK_WIDGET(active_plot));

  d=1;
}


void dynamic_plot( gpointer          data,
		   gint              source, 
		   GdkInputCondition condition )
{
  ...
  build_example1((GtkWidget *)data,p);
...
int main(int argc, char *argv[]){
  ...
  */
  ...
  build_example1(active_plot,1);
  gdk_input_add(0,
		GDK_INPUT_READ,
		dynamic_plot,
		(gpointer)active_plot);
