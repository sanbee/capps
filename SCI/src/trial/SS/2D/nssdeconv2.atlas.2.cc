...
typedef void (*sighandler_t)(int);
  ...
  {
    cerr << "#########Signal handler" << " " << signum << endl;
  }
}
...
template <class T> void TellTheWorld(int Pi, IMAGETYPE<T> &CCImg,
				     IMAGETYPE<T> &ResImg, 
				     FluxonCompList<T> &CC,
				     FluxonCompList<T> &PSF, int doit=0)
{
...
//===================================================================
//
void JustPostEvent(Client &c, const char *EventName, int Val)
{
...
int main(int argc, char **argv)
{
  ...
  sighandler_t default_sighandler;

  while (e=c.NextEvent())
    {
      default_sighandler=signal(SIGIO,signal_handler);
      ...
      else if (!strcmp(e->Name(),"restoredimg"))   
	{
      ...
      else if (!strcmp(e->Name(),"tell"))
	{
	  IMAGETYPE<FTYPE> ResImg;
	  IMAGETYPE<FTYPE> CCImg;
	  ResImg.resize(ImSize,ImSize);
	  ResImg.assign(0);
	  CCImg.resize(ImSize,ImSize);	  
	  CCImg.assign(0.0);
	  ...
	  for(int i=0;i<ImSize;i++)
	    for(int j=0;j<ImSize;j++)
	      ResImg.setVal(i,j,(DirtyImage(i,j)-ResImg(i,j)));

	  TellTheWorld(1,CCImg,ResImg,CCList,PSF,1);
	}
      else if (!strcmp(e->Name(),"smooth")) 
	{
	  IMAGETYPE<FTYPE> tmpr,img;
	  readImage(c, e, img);

	  tmpr.resize(ImSize,ImSize);
	  tmpr.assign(0);
	  for (int i=0;i<4;i++) tmpr += PSF[i];
	  Convolver<FTYPE> tConvo(tmpr);
	  tConvo.convolve(DirtyImage);
	  tConvo.Flip(DirtyImage);  
	  sendImage(c,e,DirtyImage);
	}
      else c.Unrecognized();
    }
