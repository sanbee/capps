...
using namespace std;
#define IOERROR(F)  ({if (F.fail()) {cerr << "###Error: In opening file" << endl;return 0;}})
namespace Global {
  Client *ClientPtr;
  GlishEvent  *EventPtr;
};
				   ...
				   IMAGETYPE<T> &ResImg)
{
  ...
  /*n
  Convo.convolve(ResImg);
  for(int i=EDGE;i<(N-EDGE);i++) 
    for(int j=EDGE;j<(N-EDGE);j++) 
      ResImg.setVal(i,j,(ResImg(i,j)/20.0));
  */
  //  {
    //    ofstream tmp("di.dat");
    //    ofstream tmp1("mi.dat");
    //    ofstream tmp2("ri.dat");
    //    tmp << DImg ;
    //    tmp1 << MImg;
    //    tmp2 << ResImg;
    //    exit(0);
...
#endif
      (*MComp)[k].setScale(S);
      //      (*MComp)[k].setPScale(parameterizedScale(S));
...
template <class T> void dChisq(T &dA, T &dPx, T &dPy, T &dS, void *params,int whichAsp)
{
  ...
  for (int i=0;i<Nx;i++)
	...
	tF = (*MComp)[whichAsp];
	//
	// Derivatives are evaluated only for the region bound by
	// (XRange0,YRange0) and (XRange1, YRange1).
	//
	// This range as of now is not strictly accurate for
	// derivative computation (specially for dChiSq/dScale) -
	// should be improved to take into account the multiplication
	// with the parabolid for the derivative w.r.t scale.
	//
...
#endif
	      MI[j].setScale(S);
	      //	      MI[j].setPScale(parameterizedScale(S));
	    }
	  ...
	  for (int i=0;i<Pi;i++) 
	    {
	      ...
	      if (sqrt(dA*dA+dS*dS) > 25.0) PIndex[i]=1; else PIndex[i]=-10;
	      //	      cerr << "dAsp: " << i << " " << sqrt(dA*dA+dS*dS) << " " << PIndex[i] << endl;
	    }
	  cerr << "done!";
	  ...
	  for (int i=1;i<NSCALES;i++)
	    {
	      ...
	      //	      tf.setScale(parameterizedScale(PSF[0].getScale()/pow(2.0,i)));
...
int main(int argc, char **argv)
{
  ...
  while (e=c.NextEvent())
      ...
      else if (!strcmp(e->Name(),"smooth")) 
	{
	  ...
	  //	  tf.setPScale(parameterizedScale(PSF[0].getScale()/pow(2.0,N)));
	  tf.setAmp(tf.getScale()/tf.getAmp());
	  cerr << "###Info: Smoothing residuals to " << tf.getScale() << " scale" << endl;
	  tmp.assign(0);
	  tmp += tf;
