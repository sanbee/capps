				     ...
				     int N0, int N1,int PSFSize)
{
  ...
  for (int Pi=N0; Pi<N1; Pi++)
	...
	//
	MI += f0;
	for (int i=0;i<NP;i++) if (i == Pi) PIndex[i]=1; else PIndex[i]=-1;
	...
	*/
	if (tx) gsl_vector_free(tx);
      ...
      findComponent(NIter,s,x,&my_func,MI,Pi,Pi,1,10);
      ...
      */
      ...
      MImg.assign(0.0);
      //
      // Use only the Pi th. component to compute the updated ResImg
      //
      MImg += MI[Pi];
      Convo.convolve(MImg);
