...
namespace casa {
extern Timer modelIOTimer, correctedIOTimer, weightIOTimer;
extern Double totalModelIOTime, totalCorrectedIOTime, totalWeightIOTime;
};
//
//-------------------------------------------------------------------------
//
...
void UI(Bool restart, int argc, char **argv, string& MSName, string& timeStr, string& spwStr, 
	string& antStr, string& fieldStr, float& paInc, string& cfcache, string& pointingTable,
...
int main(int argc, char **argv)
{
  ...
  //---------------------------------------------------
  //
  string MSName, timeStr, spwStr, antStr, fieldStr, cfcache,pointingTable;
 ...
 RENTER:// UI re-entry point.
  MSName=timeStr=antStr=cfcache=complist=pointingTable="";
  //
  // Factory defaults
  //
  pblimit=0.05;
  ...
  // The user interface
  //
  UI(restartUI,argc, argv, MSName, timeStr, spwStr, antStr, fieldStr, paInc, 
  ...
  LogIO os(LogOrigin("timager", "The End"));

  os << "Total MODEL_DATA IO time = " << totalModelIOTime << LogIO::POST;
  os << "Total CORRECTED_DATA IO time = " << totalCorrectedIOTime << LogIO::POST;
  os << "Total WEIGHT_DATA IO time = " << totalWeightIOTime << LogIO::POST;
	}
...
//    modelIOTimer.show(os.output(),"Total MODEL_DATA IO time: ");
//    correctedIOTimer.show(os.output(),"Total CORRECTED_DATA IO time: ");
//    weightIOTimer.show(os.output(),"Total WEIGHT_DATA IO time: ");
