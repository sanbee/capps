...
/****************Setting end*******************************************************/
  ...
  makePBImage(pb, (Int)nx, (Int)ny, 1.0, Freq2, FW2);
  Array<Float> pixels;
  ...
  for(Int i=0;i<nx;i++){
	...
	ndx(0)=i; 
	ndx(1)=j;
	val=sqrt(powl(LWA1.get_LWAs_Ereal(N), 2.0)+powl(LWA1.get_LWAs_Eimag(N), 2.0))* 
	            sqrt(powl(LWA2.get_LWAs_Ereal(N), 2.0)+powl(LWA2.get_LWAs_Eimag(N), 2.0));
	pixels(ndx)=val;
        N += 1;
	//	cout << N << " " << LWA1.get_LWAs_L(N) << " " << LWA1.get_LWAs_M(N) << " " << val << endl;
      }
  }
  pb.put(pixels);
  cout << "Powersize = " << LWA1.get_LWAs_Powersize() << endl;
